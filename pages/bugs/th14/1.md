...
[img=bugs/th14/1_comparison.png, figtitle=Two screenshots of Kagerou's third non-spell. Left side shows what it's like normally. Right side shows no bullets., alt=Two screenshots of SakuyaA fighting against Kagerou]
...
Kagerou's mid-boss spell and second spell both create invisible bullets that spawn after a few game frames. The invisible bullets count towards the bullet cap, which is set at ``2,000``. If the bullet cap of ``2,000`` is reached and enemies tries to fire its pattern, no bullets will be shot.
