...
[img=bugs/th14/2_comparison.png, figtitle=Both images are from the same replay and taken at the same time in the replay. However, the replay on the left shows a skewed laser., alt=Two game screenshot of Marisa shooting lasers. One laser is crooked.]
## [how]
[hr_minor]
...
The value of ``v19`` is set to the value of ``v48``.
However, [hl2]the value of ``v48`` is undefined[/hl2]. This is because ``v48`` [hl2]is an uninitialised variable[/hl2], meaning ``v48`` has never been assigned a value before. This means ``v48`` has an undefined value.
With this logic, the value of ``v19`` is also undefined.
This also means ``v20`` is undefined.
``v7`` sets the length of Marisa's lasers. Because ``v7`` takes the value of ``v20`` it means that ``v7`` too is undefined.
