...
MarisaB's laser can fire backward, dealing a small amount of damage to enemies that are behind her lasers. This damage only lasts for one frame.
...
[hl2]The initialized bullet state takes place for one frame before the first update frame occurs[/hl2]. This means the values of "On Initialization" are only true for one frame.
...
[box=1000]The backwards laser appears for one frame because the laser's center is at her shooter's position, and the laser's height is initialized to 480 units. This means half of the laser's hitbox is extended downward for one frame. On the next update frame, the laser is correctly repositioned higher and resized.[/box]
