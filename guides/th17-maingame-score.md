...
This is a guide for people who are interested and/or are scoring WBaWC. This will contain basic, beginner information to know how to get started, but also as much knowledge as I currently have of the game that is worth putting in a guide. 
...
f you didn't know, every mid-boss, if a special condition is met, will release a special rare beast token, if you hyper using this token, at the end of said hyper, the usual 2 beast token release is switched with a 5 token release (depending on which rare token, the 5 releases can have random beasts, life pieces and bomb pieces)
...
You want to enter with a bomb to deal as much damage as possible without depleting the hyper meter (bomb damage does NOT deplete the hyper faster) this is because PiV from shot here is not worth losing hyper length, a longer hyper is waaaaay more valuable on this spell
...
Conclusion: if you whine about bad rng, you are coping because statistically it actually helps you on average, eventually if you improve it should stop being a problem.
...
1. Prioritize timing your cancel to have the tokens you need, example: Your hyper ended, you see 3 wolves on screen, you wait 2 seconds for them to turn into otters before killing the pattern, never go into a new pattern without tokens ready, it's gonna force a bomb or a hyper skip.
...
2. Only if the tokens allow it, go for optimal bullet cancel, same example as earlier: assuming the optimal cancel happens 3 seconds after the optimal time to cancel and collect otters, PRIORITIZE TOKENS, if the tokens and optimal bullet cancel align it's good, but it's rarely necessary.
...
+ Tokens have a lifetime of 02:10 minutes, after this time they will turn gray and despawn after around 15 seconds. they can bounce as long as these 15 seconds don't end, once it reaches 15 seconds the token will just fly off the screen instead of bouncing.
+ Unlike UFOs, hugging tokens does not freeze their life timer. 
...
+ When you die and respawn, it's possible for a token to touch you when respawning if it's low on the screen, make sure you don't suicide if a token is moving in the respawn point
...
Notes for lower level: If unfocusing during stage sections with ying-yangs that shoot rng bullets is too risky for you, I would just avoid it and focus, the PiV gain is more relevant for CS+ paces...
...
Notes for lower level: as said in the stage 4 part, I highly recommend entering with 2 tokens and not 3. Doing this you have to skip the second hyper, this is because you WON'T have enough tokens to activate the second hyper in the first place. Why would you do this? Simply because the setup to do a 5 otter After mid-boss is waaaaaay harder to do if you go for the second hyper before it, second hyper gains 100M at best, I only suggest doing this for CS/11B + paces. Forgot to mention this: The rng spam section drops 2 eagles, by the time Mayumi mid-boss dies these 2 eagles will be either otters or wolves so it's veeeery consistent with the basic strat, example here: 
...
Notes for lower levels: the first hyper is usually activated during mid-boss dialogue, if you go for the early hyper make sure to practice it a lot to avoid having to consume a token.
