...
Occasionally, [hl2]the player's (sub-)shottypes do not work[/hl2] in which case their bullets are not shot. This affects the shottypes Cirno (focus and unfocus shot) and Aya (unfocus shot). This also applies to the subseasons Spring, Summer and Autumn. This issue [hl2]can be resolved by releasing [key=Z] for about half a second[/hl2].
...
Because [ins=2, n=5] has stopped, it means that the shooters that use [ins=2, n=5] are inactive, thus [hl2]those shooters do not produce any bullets/[hl2].
