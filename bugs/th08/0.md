...
When skipping through dialogue by pressing the ``Z`` button, The ``Z`` key **must be pressed**. This means that one frame before ``Z`` is "✔️", ``Z`` must be "❌". 
...
1. On ``Global timer`` = ``10``, ``Is Z button held?`` = ✔️
2. On ``Global timer`` = ``9``, ``Is Z button held?`` = ❌
...
[br] The four previously highlighted columns **are left out** intentionally. When watching a replay, the game does not take into account the button presses while pausing.
...
1. On ``In-game timer`` = ``6``, ``Is Z button held?`` = ✔️
2. On ``In-game timer`` = ``5``, ``Is Z button held?`` = ❌
