...
When a bullet is cleared (either through dying or bombing), its state variable is changed from 1 (normal) to 4 (in the process of being cleared). When the bullet's state variable is 4, any instructions set by [ins=0, n=611] are skipped. In the example of transformation ``0x04000000``, it means that any instructions of [ins=2, n=BulletEffectType-67108864] related to the ``time`` variable are skipped.

If the bullet was cleared (through dying or bombing) while it was invisible (i.e. [ins=2, n=6] is ``true``), any instructions using the ``time`` variable are skipped. This means that the flag [ins=2, n=6] can never be changed to ``false``. The implication of this is that [hl2]the bullet will forever remain in an invisible state[/hl2]. This bullet still takes up a slot in the bullet list. When enough bullets are cancelled before appearing, the bullet list eventually runs out of slots. 
<!-- 
