...
Everything is explaind in the following section. Also, [hl2]the important instructions will be highlighted[/hl2] !
...
[hl2]Instruction ``circlePos(%F0, %F1, %ANGLE + 1.0471976f, 1400.0f)`` performs the following two actions: ``F0 = cos(ANGLE + 1.0471976f) * 1400.0`` and ``F1 = sin(ANGLE + 1.0471976f) * 1400.0``[/hl2]
...
If the variable ``ANGLE_PLAYER`` were any higher, then the integer variable ``DECREMENT`` is a [hl2]much higher value[/hl2]. This means that the invisible enemy bullet spawner would have to [hl2]shoot a couple thousand bullets[/hl2] before it despawns. Furthermore, if the variable ``ANGLE_PLAYER`` [hl2]were any lower[/hl2], then the integer variable ``DECREMENT`` is a negative value. It means that the value would have to [hl2]iterate through a couple billion iterations[/hl2] before the invisible enemy bullet spawner despawns.
...
### Conclusion
[hr_minor]
[box=1000] In summary, the bullets on Dragon Eater [hl2]are shot a small number of times[/hl2] before it stops shooting. However, after 60 seconds there is an [hl2] overwhelming number of bullets[/hl2] each wave. This is because part of the [hl2]code is accessed differently[/hl2]. Instead, [hl2]the amount of bullets that are shot depends on your player angle[/hl2]. [/box]
