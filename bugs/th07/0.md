...
If you ever want to replicate the final spell-card Merlin glitch yourself, remember that on average on Easy/Normal it would take [hl2]787 attempts[/hl2], and on Hard/Lunatic it would take [hl2]854 attempts[/hl2].
...
For the post-midboss Merlin glitch to happen, [hl2]exactly three out of four of the highlighted subs[/hl2] must exist in the stack when doing the glitch. If there are less than three, then the glitch does not happen. If there are four, then the [jumpto=#/bugs/th07/0#merlinsfullcallstackfinalspellcardglitch]final spell-card glitch[/jumpto] happens.
[hr_minor]
...
The probabilities were calculated as follows:
For Easy/Normal mode, the probability of getting the call stack ``sub71 → sub72 → [...] → sub143 → sub2`` with 3 out of 4 subs being true is as follows:
[code]P((E/N sub71 → sub72 → [...] → sub143 → sub52) with 3/4 subs true)
= P(sub80 → sub81 → sub2) * P(E/N sub121 → sub124) * P(sub83 → sub84 → sub2) * P(sub138 ⇏ sub2)
+ P(sub80 → sub81 → sub2) * P(E/N sub121 → sub124) * P(sub83 → sub84 ⇏ sub2) * P(sub138 → sub2)
+ P(sub80 → sub81 → sub2) * P(E/N sub121 ⇏ sub124) * P(sub83 → sub84 → sub2) * P(sub138 → sub2)
+ P(sub80 → sub81 ⇏ sub2) * P(E/N sub121 → sub124) * P(sub83 → sub84 → sub2) * P(sub138 → sub2)
= 80/690 * 432/472 * 96/740 * 788/868 + 80/690 * 432/472 * 644/740 * 80/868 + 80/690 * 40/472 * 96/740 * 80/868 + 610/690 * 432/472 * 96/740 * 80/868
= 1677952 / 54476765
≈ 3.080%
≈ 1/32[/code]

For Lunatic/Hard mode, the probability of getting the call stack ``sub71 → sub72 → [...] → sub143 → sub2`` with 3 out of 4 subs being true is as follows:
...
+ P(sub80 → sub81 → sub2) * P(H/L sub121 ⇏ sub124) * P(sub83 → sub84 → sub2) * P(sub138 → sub2)
+ P(sub80 → sub81 ⇏ sub2) * P(H/L sub121 → sub124) * P(sub83 → sub84 → sub2) * P(sub138 → sub2)
= 80/690 * 432/512 * 96/740 * 788/868 + 80/690 * 432/512 * 644/740 * 80/868 + 80/690 * 80/512 * 96/740 * 80/868 + 610/690 * 432/512 * 96/740 * 80/868
= 26318 / 923335
≈ 2.850%
≈ 1/35[/code]

If you ever want to replicate the post-boss Merlin glitch yourself, it would take [hl2]32 attempts[/hl2] on Easy/Normal, and it would take [hl2]35 attempts[/hl2] on Hard/Lunatic.
